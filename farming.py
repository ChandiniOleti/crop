    ...
    if uploaded_file:
        ...
        with col2:
            with st.spinner("🧠 Analyzing..."):
                ...
                result, confidence = predict_disease(img_bytes)
                
                if result is None:
                    st.error("❌ This doesn't appear to be a plant leaf image. Please upload a clear image of a plant leaf.")
                    st.info(f"Confidence level: {confidence*100:.2f}% (Below threshold)")
                else:
